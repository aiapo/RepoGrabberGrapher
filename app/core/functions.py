...
import hashlib
import re
from app.db.models import Imports, Queries, Repositories
from app.core.routes.api.refactorings import refactoringsBy
...
def sendToDBCore(rows, conn: sqlite3.Connection):
    if rows:
        if any(d["relation"] == "repositories" for d in rows):
            conn.executemany(
                "INSERT INTO repositories VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT DO NOTHING",
                [
                    (
                        x["data"][0],
                        x["data"][1],
                    ...
                    for x in rows
                    if x["relation"] == "repositories"
                ],
            )

        if any(d["relation"] == "languages" for d in rows):
            conn.executemany(
                "INSERT INTO languages VALUES (?,?,?) ON CONFLICT DO NOTHING",
                [
                    (x["data"][0], x["data"][1], int(x["data"][2]))
                    for x in rows
                    if x["relation"] == "languages"
                ],
            )

        if any(d["relation"] == "refactorings" for d in rows):
            conn.executemany(
                "INSERT INTO refactorings VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT DO NOTHING",
                [
                    (
                        x["data"][0],
                        x["data"][1],
...
def readRGDS(fileName: str, compressed: bool, db_name: str):
    ...
    if compressed:
        ...
        totalLines = sum(
            1 for line in gzip_ng_threaded.open(fileName, "rt", encoding="utf8")
        )
                ...
                if not line.lstrip().startswith("%"):
                    ...
                    if line.startswith("@"):
                        ...
                        if validAction.get(currentAction) != None:
                            ...
                            if validAction.get(currentAction) == len(currentActionFull):
                                ...
                                print(
                                    "[ERROR] Invalid number of parameters for @Action "
                                    + currentAction
                                    + " in RGDS file... ignoring"
                                )
                        else:
                            print(
                    ...
                    # if are in data reading and the line isn't empty
                        ...
                        for i in range(len(currentDataLine)):
                            ...
                            currentDataLine[i] = re.sub(
                                r'^"|"$', "", currentDataLine[i]
                            )
                        # add to list of rows to be added to the specified relation
                        rows.append(
                            {"relation": currentRelation, "data": currentDataLine}
                        )
...
def parseArgs(argv):
    ...
    parser.add_argument(
        "--rgds", help="Load Repository Dataset from RGDS File", required=True
    )
