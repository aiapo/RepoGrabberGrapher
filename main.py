...
from app.core.functions import parseArgs, readRGDS
        ...
        if opts.rgds:
            ...
            def importDs(ds):
                if readRGDS(ds, True, DB_NAME):
                    ...
                    print(
                        "Dataset ignored because RGDS file has already been imported. Using database..."
                    )
            ...
            if validators.url(url):
                ...
                if dl != None and genericDl != None:
                    if not foundDl:
                        print(
                            domain
                            + " doesn't have it's own Downloader, using Generic..."
                        )
                        dl = genericDl
                    else:
                        print(
                            "Found a downloader for "
                            + domain
                            + " with "
                            + dl.NAME
                            + " Downloader..."
                    ...
                    if dl.checkExists(url):
                        if dl.checkDir(url):
                            ...
                            datasets.append({"location": ""})
                        print(datasets)
                        for dataset in datasets:
                            dl.download(
                                dl.getDownloadUrl(url, dataset["location"]), path
                            )
                            importDs(path)
                    ...
                    print(
                        "No downloaders found, not even the generic. Check your files."
                    )
                    exit(1)
            else:
